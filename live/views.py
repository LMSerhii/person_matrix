...
def second(request):
    ...
    return render(request, 'second_variable.html', context=context)


def result_2(request):
    if request.method == 'GET':
        birthday = request.GET.get('birth')
        res = birthday.split('-')
        month = num_sum(res[1])  # первый сверху
        day = num_sum(res[2])  # первый слева
        year = num_sum(res[0])  # первый справа
        f_1 = num_sum((day + month + year))  # Первый снизу
    ...
    return render(request, 'result_2.html', context=context)
